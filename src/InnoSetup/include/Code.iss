...
const
	...
	AddinsRegistry = 'Software\6i\{#SetupSetting("AppID")}';
...
var
  ...
  ResultStr: string;
  ResultCode: Integer;
  ResultDisplayVersion: string;
begin
  // Check if the application is already install
  // MsgBox('Application Id = ' + '{#AppID}', mbInformation, mb_Ok);
  begin
    If RegQueryStringValue(HKLM, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{#AppID}_is1', 'UninstallString', ResultStr) then begin
      If ResultStr<>'' then begin
        ResultStr:=RemoveQuotes(ResultStr);
          RegQueryStringValue(HKLM, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{#AppID}_is1', 'DisplayVersion', ResultDisplayVersion)
          if MsgBox(FmtMessage(CustomMessage('AlreadyInstall'), ['MicrosoftOfficeExtensions', ResultDisplayVersion]), mbConfirmation, MB_YESNO) = idYes then
...
begin
	...
	if OkToCopyLog then
		FileCopy(
			ExpandConstant('{log}'),
			AddBackslash(GetDestDir(''))+'\{#logfile}',
			false);
	{
